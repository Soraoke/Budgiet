...
internal object RecentCurrencies {
    ...
    fun get(): State<Result<List<String>>?> {
        ...
        this.state = rememberWork {
            ...
            file.readText()
                ...
                .dropLast(1)
                .toMutableStateList()
                .apply { Log.i(RECENT_CURRENCIES_LOG_TAG, "Loaded recent currencies from storage.") }
        }

        if (this.state.value is Result.Err) {
            Log.e(RECENT_CURRENCIES_LOG_TAG, "Error reading recent currencies from storage: ${(this.state.value as Result.Err).error}")
        }
    ...
    fun clear(context: Context) {
        if (!this::state.isInitialized) {
            Log.w(RECENT_CURRENCIES_LOG_TAG, "RecentCurrencies.state must be initialized before calling clear() or moveToFront().")
            return
        }
     ...
     *
     * See [RecentCurrencies.get] to read from this [List]. */
    fun moveToFront(currencyCode: String, context: Context) {
        if (!this::state.isInitialized) {
            Log.w(RECENT_CURRENCIES_LOG_TAG, "RecentCurrencies.state must be initialized before calling clear() or moveToFront().")
            return
        }
        ...
        val orderedCurrencies = this.state.value!!.unwrap() as MutableList<String>
